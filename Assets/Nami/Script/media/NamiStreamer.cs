...
public class NamiStreamer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        pc.OnIceGatheringStateChange = (state) =>
            {
                ...
                {
                    // StartCoroutine(ExchangeSDP(url, offerSdp));
                    Debug.Log("------------RTCIceGatheringState.Complete-----------------");
                    Debug.Log(this.offerSdp);
                    StartCoroutine(ExchangeSDP(url, offerSdp));
                }
            };
        ...
        IEnumerator OnCreateOfferSuccess(RTCSessionDescription offer)
        {
            ...
            offerSdp = offer.sdp;
            Debug.Log($"WebRTC: SetLocalDescription {offer.type} {offerSdp}");
            yield return op;
        ...
        IEnumerator ExchangeSDP(string url, string offer)
        {
            ...
            var task = System.Threading.Tasks.Task<string>.Run(async () =>
            {
                ...
                {
                    Debug.Log($"WebRTC: No candidates");
                }

                for (int i = 0; i < candidates.Count; i++)
                {
                    var can = candidates[i];
                    offerSdp += "a=" + can.ToString();
                    offerSdp += "\r\n";
                }

                Debug.Log(offerSdp);
                var content = new System.Net.Http.StringContent(offerSdp);
            ...
            {
                Debug.LogError($"WebRTC: Exchange SDP failed, url={url}, err is {task.Exception.ToString()}");
                yield break;
            }
