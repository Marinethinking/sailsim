...
- **Telemetry bus (simulated CAN / NMEA2000)**
  - Multicast endpoint `239.10.0.1:20000` (`UdpTelemetryConfig.TelemetryEndpoint`).
...
- **Control channel**
  - UDP unicast port `20101` (`UdpTelemetryConfig.ControlListenEndpoint`).
  - Expected PGNs:
...
- Per-script UDP address fields have been removed; all telemetry producers now broadcast on the shared multicast endpoint defined in `UdpTelemetryConfig`.
