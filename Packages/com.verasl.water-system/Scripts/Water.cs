...
namespace WaterSystem
{
    ...
    public class Water : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            // Defer depth capture until SRP is initialized to avoid re-entrant pipeline creation
            if (Application.platform != RuntimePlatform.WebGLPlayer && Application.isPlaying)
            {
                if (_delayedDepthCaptureCoroutine != null)
                {
                    StopCoroutine(_delayedDepthCaptureCoroutine);
                }
                _delayedDepthCaptureCoroutine = StartCoroutine(DelayedCaptureDepthMap());
            }
        }
        ...
        public void Init()
        {
            ...
            // Depth capture is deferred in OnEnable for play mode
        }
        ...
        {
            // If SRP is not yet initialized, skip
        ...
        private Coroutine _delayedDepthCaptureCoroutine;
        private IEnumerator DelayedCaptureDepthMap()
        {
            // Wait until SRP is created then one frame
            while (UnityEngine.Rendering.GraphicsSettings.currentRenderPipeline == null ||
            ...
            CaptureDepthMap();
            _delayedDepthCaptureCoroutine = null;
        }
